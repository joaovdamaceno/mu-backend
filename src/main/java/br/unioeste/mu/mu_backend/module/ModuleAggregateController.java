...
public class ModuleAggregateController {
    ...
    public ModuleAggregateResponse create(
            @Valid
            @RequestBody(description = "Payload para criação agregada de módulo", required = true, content = @Content(
                    examples = @ExampleObject(value = """
                            {
                              "title": "Introdução à Programação",
                              "notes": "Módulo inicial para novos alunos",
            ...
            @org.springframework.web.bind.annotation.RequestBody ModuleAggregateRequest request
    ) {
